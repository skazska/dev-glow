...
pub fn validate_config<T: serde::Serialize>(value: &T, schema_json: &str) -> Result<()> {
    ...
    if let Err(errors) = result {
        let error_messages: Vec<String> = errors.map(|e| format!("{}: {}", e.instance_path, e)).collect();
...
mod tests {
    ...
    fn test_validate_default_config() {
        ...
        assert!(validate_config(&config, CONFIG_SCHEMA).is_ok());
    }

    #[test]
    fn test_validate_default_process_config() {
        let config = ProcessConfig::default();
        assert!(validate_config(&config, PROCESS_CONFIG_SCHEMA).is_ok());
    }
}
